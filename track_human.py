#!/usr/bin/env python
"""
track_human.py -- Continuous Person Tracking with SO-100 Arm
=============================================================
Detects a person in the camera frame using YOLO and continuously
steers the arm end-effector toward them using IK from move_to_3d_location.py.
...
PERSON_WIDTH_M    = 0.45   # typical shoulder width for depth estimate
...
def estimate_depth(bbox_width_px, fx):
    if bbox_width_px < 1:
        return float("inf")
    return (PERSON_WIDTH_M * fx) / bbox_width_px
...
class DetectionThread(threading.Thread):
    ...
    def run(self):
        # person class id
        person_ids = [cid for cid, n in self.model.names.items() if n == "person"]
        classes_arg = person_ids if person_ids else None
        ...
        while self.running:
            ...
            if best_box is not None:
                ...
                cx_px = (x1 + x2) / 2.0
                cy_px = (y1 + y2) / 2.0   # centre of person bbox
...
def parse_args():
    p = argparse.ArgumentParser(
        description="Track a person with the SO-100 arm using YOLO + IK."
    )
...
def main():
    ...
    person_ids = [cid for cid, n in model.names.items() if n == "person"]
    if person_ids:
        print(f"[INFO] Model ready -- tracking: person  (YOLO IDs: {person_ids})")
    else:
        print("[WARN] 'person' class not found in model -- will attempt anyway.")
        ...
        while det.running:
            ...
            if use_arm:
                ...
                if tgt is not None:
                    ...
                    if not (np.isnan(x_m) or np.isnan(y_m)):
                        ...
                        if sol is not None:
                            ...
                            if max_delta > 45.0:
                                ...
                                joints = sol
                                print(
                                    f"[TRACK] person X={x_m:.3f} Y={y_m:.3f}  "
                    ...
                    # No person detected -- hold position
                    robot.send_action(build_action(joints, motor_names, home_last2))
            ...
            if not args.no_window:
                ...
                if frame is not None:
                    ...
                    label = f"HUMAN TRACKING  zoom {zoom:.1f}x"
                    cv2.putText(frame, label,
                                (CAM_WIDTH - 210, 18),
                                cv2.FONT_HERSHEY_SIMPLEX, 0.42, (0, 200, 255), 1)
                    cv2.imshow("LAMP Human Tracking", frame)
